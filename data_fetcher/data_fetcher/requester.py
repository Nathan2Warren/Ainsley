...
class GraphQLClient:
    ...
    def __get_number_of_profile_ids(self):
        response = requests.post(
            self.url, json={"query": global_protocol_stats_query}
        )
    ...
    def _get_data_for_all_profiles(self, limit: int = 50):
        ...
        while prev != _next:
            ...
            response = requests.post(
                self.url, json={"query": profile_query, "variables": variables}
            )
            if response.status_code == 200:
                content = json.loads(response.content)
    ...
    def get_publications(self, profile_id: str, limit: int = 50):
        ...
        response = requests.post(
            self.url,
            json={"query": publications_query, "variables": variables},
        )
        if response.status_code == 200:
            content = json.loads(response.content)
